<?php 
	...
	if (isset($_POST['_action_']) && $_POST['_action_'] == 'add_news') {
        ...
        if($_FILES['picture']['error'] == UPLOAD_ERR_OK && $_FILES['picture']['name'] != "") {
			...
			if ($ext == '.jpg' || $ext == '.png' || $ext == '.gif') { # test if format is picture
				...
				$_SESSION['message'] .= '<p>Uspiješno dodana slika.</p>';
			}
        }
		
		
		$_SESSION['message'] .= '<p>Vijest je uspiješno dodana!</p>';
	...
	if (isset($_POST['_action_']) && $_POST['_action_'] == 'edit_news') {
		...
		$_SESSION['message'] = '<p>Vijest je uspiješno izmijenjena!</p>';
	...
	if (isset($_GET['delete']) && $_GET['delete'] != '') {
		...
		$_SESSION['message'] = '<p>Uspiješno ste obrisali vijest!</p>';
	...
	#Add news 
		...
		<form action="" id="news_form" name="news_form" method="POST" enctype="multipart/form-data">
			...
			<textarea id="description" name="description" placeholder="Sadržaj vijesti.." required></textarea>
